...
- 音声ファイル（ノイズ・BGM等があっても可、複数話者可、複数ファイルでも単一ファイルでも）を`01-Sources`フォルダに入れる
- `1-CreateSegments.bat`をダブルクリックして待つ。ノイズ・BGM除去をしなくてもよい場合は`1-CreateSegmentsNoDemucs.bat`をダブルクリック。
- `2-CreateDatasetsUI.bat`をダブルクリック。WebUIが立ち上がるので、データセットの修正を行う。もしデータを別に修正しなくてもいい場合は、`2-CreateDatasetsAuto.bat`をダブルクリック。
- `3-CheckDataset.bat`をダブルクリック。これにより出来上がったデータセットの合計時間等が確認できる。

出来上がったものは`04-Datasets`内の話者名のフォルダに入っています。
